...
# from scipy.integrate import quad # quad is no longer used
from scipy.interpolate import interp1d # Still needed for T(E) interpolation
import scipy.constants as const
import logging

# --- System Parameters & Input Data ---
DIST_LI_LI = 2.9000  # Angstroms, Li-Li distance
LEFT_LEAD_ATOMS = 3
CENTRAL_REGION_ATOMS = 8
RIGHT_LEAD_ATOMS = 3
ORBITALS_PER_ATOM = 1 
CALCULATED_FERMI_ENERGY_eV = -0.811928 

# On-site energies (eV) for all atoms in the chain
ON_SITE_ENERGIES_eV = [
...
# Nearest-neighbor hopping terms (eV)
HOPPING_TERMS_eV = [
...
TOTAL_ATOMS = LEFT_LEAD_ATOMS + CENTRAL_REGION_ATOMS + RIGHT_LEAD_ATOMS
if len(ON_SITE_ENERGIES_eV) != TOTAL_ATOMS:
    raise ValueError(f"ON_SITE_ENERGIES_eV length mismatch: expected {TOTAL_ATOMS}, got {len(ON_SITE_ENERGIES_eV)}.")
if len(HOPPING_TERMS_eV) != TOTAL_ATOMS - 1:
    raise ValueError(f"HOPPING_TERMS_eV length mismatch: expected {TOTAL_ATOMS - 1}, got {len(HOPPING_TERMS_eV)}.")
if ORBITALS_PER_ATOM != 1:
    logging.warning(f"ORBITALS_PER_ATOM is {ORBITALS_PER_ATOM}. This script is designed for 1 orbital per atom.")

# --- Physical Constants ---
G0 = 2 * const.e**2 / const.h  # Conductance quantum (S)
KB_J_PER_K = const.k           # Boltzmann constant (J/K)
EV_TO_JOULE = const.e          # eV to Joules conversion

# --- Simulation Settings ---
# MODIFIED: Strict usage of CALCULATED_FERMI_ENERGY_eV
if CALCULATED_FERMI_ENERGY_eV is None or np.isnan(CALCULATED_FERMI_ENERGY_eV):
    logging.error("CALCULATED_FERMI_ENERGY_eV is not valid (None or NaN). Please provide a valid Fermi energy.")
    raise ValueError("Invalid CALCULATED_FERMI_ENERGY_eV.")
else:
    FERMI_ENERGY_eV = CALCULATED_FERMI_ENERGY_eV
    logging.info(f"Using strictly defined Fermi Energy: {FERMI_ENERGY_eV:.4f} eV")

TEMPERATURE_K = 0.1  # Temperature for finite-T conductance calculation (Kelvin)

# --- Fermi-Dirac Distribution Functions ---
def fermi_dirac(energy_ev, chemical_potential_ev, temperature_k):
    if temperature_k < 1e-6: 
        return 1.0 if energy_ev < chemical_potential_ev else 0.0 if energy_ev > chemical_potential_ev else 0.5
    kt_ev = KB_J_PER_K * temperature_k / EV_TO_JOULE
    if kt_ev == 0: 
        return 1.0 if energy_ev <= chemical_potential_ev else 0.0
    exp_arg = np.clip((energy_ev - chemical_potential_ev) / kt_ev, -700, 700)
    return 1.0 / (1.0 + np.exp(exp_arg))

def fermi_dirac_derivative(energy_ev, chemical_potential_ev, temperature_k):
    if temperature_k < 1e-6: 
...
def make_system(lattice_spacing=DIST_LI_LI, num_orbitals=ORBITALS_PER_ATOM):
    lat = kwant.lattice.chain(lattice_spacing, norbs=num_orbitals)
    syst_builder = kwant.Builder()

    central_kwant_sites = []
    if CENTRAL_REGION_ATOMS > 0:
        for i in range(CENTRAL_REGION_ATOMS):
            atom_idx_in_chain = LEFT_LEAD_ATOMS + i
            central_site_tag = i 
            site = lat(central_site_tag)
            central_kwant_sites.append(site)
            syst_builder[site] = ON_SITE_ENERGIES_eV[atom_idx_in_chain]
        
        for i in range(CENTRAL_REGION_ATOMS - 1):
            hopping_idx_in_chain = LEFT_LEAD_ATOMS + i
            syst_builder[central_kwant_sites[i], central_kwant_sites[i+1]] = HOPPING_TERMS_eV[hopping_idx_in_chain]

    inter_cell_hopping_eV = HOPPING_TERMS_eV[0] 
    logging.info(f"Lead inter-cell hopping: {inter_cell_hopping_eV:.4f} eV (from Atom 0-1)")
    lead_unit_cell_sites = 1 

    if LEFT_LEAD_ATOMS > 0:
        ...
        lead_site_tag = 0 
        left_lead_builder[lat(lead_site_tag)] = ON_SITE_ENERGIES_eV[0] 
    ...
    if RIGHT_LEAD_ATOMS > 0:
        ...
        lead_site_tag = 0
        first_atom_right_lead_idx = LEFT_LEAD_ATOMS + CENTRAL_REGION_ATOMS 
        right_lead_builder[lat(lead_site_tag)] = ON_SITE_ENERGIES_eV[first_atom_right_lead_idx]
        right_lead_builder[lat(lead_site_tag + lead_unit_cell_sites), lat(lead_site_tag)] = inter_cell_hopping_eV
        if CENTRAL_REGION_ATOMS > 0:
            syst_builder.attach_lead(right_lead_builder, central_kwant_sites[CENTRAL_REGION_ATOMS-1])
        elif LEFT_LEAD_ATOMS == 0: 
             syst_builder.attach_lead(right_lead_builder)
    return syst_builder.finalized()

# --- DOS Calculation for Central Region ---
def calculate_total_dos_central_region(
    ...
    if orbitals_per_atom <= 0:
        logging.error(f"Invalid orbitals_per_atom ({orbitals_per_atom}). Cannot calculate DOS.")
        return np.full_like(energies_ev, np.nan)

    dos_values = np.full_like(energies_ev, np.nan)
    expected_ldos_array_size = num_central_atoms_defined * orbitals_per_atom 
            ...
            if ldos_contributions.size == expected_ldos_array_size:
                dos_values[i] = np.sum(ldos_contributions)
                 ...
                 logging.warning(
                    ...
                    f"Expected {expected_ldos_array_size} for defined central region, "
...
def compute_conductance_metrics(
    ...
    conductance_T0_S = np.interp(fermi_energy_ev, valid_energies, valid_transmission) * conductance_quantum
        ...
        if integration_min < integration_max:
            try:
                integral_val = midpoint_integration(
                    ...
                    args=(fermi_energy_ev, temperature_k) 
                )
                if not np.isnan(integral_val):
                    conductance_finite_T_S = conductance_quantum * integral_val
            except Exception as e: 
...
# --- Main Execution ---
def main():
    if CENTRAL_REGION_ATOMS == 0 and (LEFT_LEAD_ATOMS == 0 or RIGHT_LEAD_ATOMS == 0):
    ...
    energy_min_ev = FERMI_ENERGY_eV - 4.0
    energy_max_ev = FERMI_ENERGY_eV + 4.0
    energies_ev = np.linspace(energy_min_ev, energy_max_ev, 200)
    ...
    dos_values = calculate_total_dos_central_region(
        finalized_syst, energies_ev, CENTRAL_REGION_ATOMS, ORBITALS_PER_ATOM
    )

    G_T0_at_EF, G_finite_T_at_EF = compute_conductance_metrics(
        FERMI_ENERGY_eV, TEMPERATURE_K, energies_ev, transmission_values, 
        G0, energy_min_ev, energy_max_ev,
        num_integration_intervals=500 
    )
    if len(finalized_syst.leads) < 2: 
        G_T0_at_EF, G_finite_T_at_EF = np.nan, np.nan
        logging.info("Skipping conductance value calculation: Less than 2 leads.")

    logging.info(f"G(T=0K, E_F={FERMI_ENERGY_eV:.4f}eV): {G_T0_at_EF:.4e} S")
    logging.info(f"G(T={TEMPERATURE_K}K, E_F={FERMI_ENERGY_eV:.4f}eV): {G_finite_T_at_EF:.4e} S")
    ...
    if not np.isnan(G_T0_at_EF):
        axs[2].axhline(G_T0_at_EF, color='blueviolet', linestyle=':', 
                       label=f'G($E_F$, T=0K) = {G_T0_at_EF:.2e} S')
    if not np.isnan(G_finite_T_at_EF):
        axs[2].axhline(G_finite_T_at_EF, color='deeppink', linestyle='-.', 
                       label=f'G($E_F$, T={TEMPERATURE_K}K) = {G_finite_T_at_EF:.2e} S')
